...
void CData ::FilterSamples(void)
{
  ...
  if (rmsamples.size() == 0 && selectedsamples.size() == 0)
    return;
  nsp = 0;
...
void CData ::ReadData(const char *fnhap, const char *fnsample)
{
  ...
  while (getline(inz, line))
  {
    ...
    if (rmposs.size() != 0 && binary_search(rmposs.begin(), rmposs.end(), e.pos))
      continue; // skip removed snps
    if (selectedposs.size() != 0 && !binary_search(selectedposs.begin(), selectedposs.end(), e.pos))
      continue; // skip snps outside selectedposs
