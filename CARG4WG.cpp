...
void CARG4WG ::CheckCount(void) const
{
  ...
  for (int i = 0; i != nsite; ++i)
  {
    ...
    for (int j = 0; j != (int)activenodes.size(); ++j)
    {
      ...
      if (nodes[id]->ExistSite(i))
        s = s + nodes[id]->GetSite(i);
    }
...
void CARG4WG ::ReadData(const CData *idata)
{
  ...
  free(tempc);
  for (int k = 0; k != nsite; ++k)
    if (POSSIBLEMUT(count0[k], count1[k]))
      mutlist.push_back(k);
}
...
bool CARG4WG ::PossibleCoal(void)
{
  ...
  while (!coallist.empty())
  {
    ...
    if (node1->GetMark() || node2->GetMark())
      continue;
    Coal(node1, node2);
    return true;
  }
...
bool CARG4WG ::Recom(void)
{
  ...
  if (priqueue->size() > 50000000 && priqueue->size() > activenodes.size() * activenodes.size() * 10)
  {
    ...
    while (!priqueue->empty())
    {
      ...
      priqueue->pop();
      if (!e.node1->GetMark() && !e.node2->GetMark())
        temp->push(e);
    }
  ...
  for (int i = 0; i != (int)qe.size(); ++i)
    if (!qe[i].node1->GetMark() && !qe[i].node2->GetMark())
      priqueue->push(qe[i]);
  if ((temp.match != 0) && !temp.node1->GetMark() && !temp.node2->GetMark())
    priqueue->push(temp);
  return true;
}
...
void CARG4WG ::Check(const COneNode *root, int siteid) const
{
  ...
  while (i < (int)q.size())
  {
    ...
    while (1 == 1)
    {
      ...
      int nc = 0;
      if (c1 != NULL && c1->Contain(siteid))
      {
        nc++;
        p = c1;
      }
      if (c2 != NULL && c2->Contain(siteid))
      {
        nc++;
        p = c2;
      }
